...
export default function XPVisa() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = xpvisa
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "xpvisa"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "xpvisa", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <>
      ...
      <CardBody>
        <Description onAdd={handleAddTransaction} bankId='xpvisa' />
      </CardBody>
    </>
  );
}

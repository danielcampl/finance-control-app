...
export default function Nubank() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = nubank
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "nubank"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "nubank", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <CardBody>
      ...
      <Description onAdd={handleAddTransaction} bankId='nubank' />
    </CardBody>
  );
}

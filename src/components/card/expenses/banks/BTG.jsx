...
export default function BTG() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = btg
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "btg"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "btg", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <>
      ...
      <CardBody>
        <Description onAdd={handleAddTransaction} bankId='btg' />
      </CardBody>
    </>
  );
}
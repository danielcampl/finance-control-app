...
export default function Pix() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = pix
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "pix"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "pix", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <>
      ...
      <CardBody>
        <Description onAdd={handleAddTransaction} bankId='pix' />
      </CardBody>
    </>
  );
}

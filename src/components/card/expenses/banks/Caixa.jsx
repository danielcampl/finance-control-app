...
export default function Caixa() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = caixa
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "caixa"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "caixa", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <>
      ...
      <CardBody>
        <Description onAdd={handleAddTransaction} bankId='caixa' />
      </CardBody>
    </>
  );
}
...
export default function Santander() {
  ...
  useEffect(() => {
    ...
    // busca apenas transações do usuário logado e com bankId = santander
    const col = collection(db, "users", user.uid, "transactions");
    const q = query(col, where("bankId", "==", "santander"));
  ...
  const handleAddTransaction = async (transaction) => {
    ...
    await addDoc(col, {
      ...transaction,
      bankId: "santander", // já definido pelo componente
      userId: user.uid,
    });
  };
  ...
  return (
    <>
      ...
      <CardBody>
        <Description onAdd={handleAddTransaction} bankId='santander' />
      </CardBody>
    </>
  );
}